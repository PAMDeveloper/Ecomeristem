...
void Model::compute(double t, bool /* update */)
{
    ...
    if (ler_model.is_computed(t, Ler::LER) and
        ...
        exp_time_model.put(t, ExpTime::LER, ler_model.get < double >(t, Ler::LER));
    ...
    if (exp_time_model.is_computed(t, ExpTime::EXP_TIME) and
        ...
        len_model.put(t, Len::LER, ler_model.get < double >(t, Ler::LER));
    ...
    if (len_model.is_computed(t, Len::LEN)) {
        width_model.put(t, Width::LEN, len_model.get < double >(t, Len::LEN));
        width_model(t);
    }
    ...
    if (manager_model.is_computed(t, Manager::LEAF_PHASE)) {
        ...
        thermal_time_since_ligulation_model.put(
            ...
            manager_model.get < double >(t, Manager::LEAF_PHASE));
        thermal_time_since_ligulation_model(t);
    }

    if (len_model.is_computed(t, Len::LEN)) {
        blade_area_model.put(t, BladeArea::LEN, len_model.get < double >(t, Len::LEN));
