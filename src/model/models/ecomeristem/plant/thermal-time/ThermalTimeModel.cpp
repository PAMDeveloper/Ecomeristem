...
ThermalTimeModel::ThermalTimeModel()
{
    // submodels
    ...
    external(TA, &ThermalTimeModel::_Ta);
    external(GROW, &ThermalTimeModel::_grow);
    external(LIG, &ThermalTimeModel::_lig);
    external(PLASTO_DELAY, &ThermalTimeModel::_plasto_delay);
}

void ThermalTimeModel::init(double t, const model::models::ModelParameters& parameters)
{
...
void ThermalTimeModel::compute(double t, bool /* update */)
{
