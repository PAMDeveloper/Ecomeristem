...
function ImportLeaf_LE(const name : string ; const Lef1, MGR, plasto, ligulo, WLR, LL_BL, allo_area, G_L, Tb, resp_LER, coeff_lifespan, mu : double ; const isFirstLeaf : Double = 0 ; const isOnMainstem : Double = 0) : TEntityInstance;
...
begin
  ...
  entityLeaf.InternalConnect(['predimOfPreviousLeaf',
  ...
  procTmp.ExternalConnect(['isFirstLeaf','isOnMainstem','predimOfPreviousLeaf','predimLeafOnMainstem','Lef1','MGR','testIc','fcstr','predim']);
  ...
  procTmp.ExternalConnect(['FTSW','thresLER','slopeLER','P','resp_LER','reductionLER']);
  ...
  procTmp.ExternalConnect(['isFirstLeaf','isOnMainstem','predim','len','LER','exp_time']);
  ...
  procTmp := TProcInstanceInternal.Create('computeReductionLER',ComputeLER_LEDyn,['FTSW','kIn','Thres','kIn','slope','kIn','P','kIn','resp_LER','kIn','reductionLER','kOut']);
  procTmp.SetProcName('ComputeLER_LE');
  procTmp.SetExeStep(1); // pas journalier
  ...
  procTmp := TProcInstanceInternal.Create('computeExpTimeRealization',ComputeLeafExpTime_LEDyn,['isFirstLeaf','kIn','isOnMainstem','kIn','predim','kIn','len','kIn','LER','kIn','exp_time','kOut']);
  procTmp.SetProcName('ComputeLeafExpTime_LE');
  procTmp.SetExeStep(1); // pas journalier
  ...
  procTmp.ExternalConnect(['zero','one','predim','len','LER','exp_time']);
  ...
  procTmp := TProcInstanceInternal.Create('computeExpTimeLigule',ComputeLeafExpTime_LEDyn,['isFirstLeaf','kIn','isOnMainstem','kIn','predim','kIn','len','kIn','LER','kIn','exp_time','kOut']);
  procTmp.SetProcName('ComputeLeafExpTime_LE');
  procTmp.SetExeStep(1); // pas journalier
 ...
 // connection port <-> attribut pour 'correctedBladeArea'
  ...
  // ajout d'un manageur
  ////////////////////////////////////////////////////////
  managerTmp := TManagerInternal.Create('leafManager_LE',LeafManager_LE);
  managerTmp.SetFunctName('LeafManager_LE');
